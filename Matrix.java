...
public abstract class Matrix {
    ...
    public String toString() {
        //firstly create empty string
        String newString = "";
        for (int i = 0; i < iDim; i++) {
            
            for (int j = 0; j < jDim; j++) {
                //for each position in the matrix the corresponding value is added to a string to 2dp
                newString = String.format(newString + "%.2f",getIJ(i, j));
                if (j < jDim-1) {
                    //then we put spaces after each value 
                    newString=newString + "  ";
                }
            }
            //creates a new line at the end of each row
            newString=String.format(newString + "%n");
        }
        return newString;
    }
